...
- app の dispatch がコマンド成功後 (`advance` も含む) に `JSON.stringify(state)` を書く。同期書き込みが重い環境では直近 1 件だけを保留してデバウンスするが、**次のコマンドの前には必ず書き終わっている** こと
- 演出の途中 (バトルのステップの途中) で落ちても、`battle.step` が保存されているので、再開時に StepMover が続きのステップを演出つきで進める。ステップとステップの間の状態は常に正しいゲーム状態 (02 の原則 9)
